...
#from random import randint as foorandint
...
class Station:
  ...
  def physics(self):
    ...
    if self.progress == 0:
      ...
      self.progress = TPS * tubeAmmos[self.producing].buildCost
    floats = c_float * 3
    myvar = floats(self.location[0], self.location[1], self.location[2])
    ChangeValue( server_handle, CreateCent( bytes("/e/sta/"+self.name+"/loc", 'utf-8'), 0x80, 1, 12, myvar ), 1 );
...
class enemyShip():
  ...
  def physics(self):
    ...
    floats = c_float * 3
    myvar = floats(self.location[0], self.location[1], self.location[2])
    ChangeValue( server_handle, CreateCent( bytes("/e/ene/"+self.name+"/loc", 'utf-8'), 0x80, 1, 12, myvar ), 1 );
  def damage(self, damage, sourceType, source, location, frequency):
    angle = Angle(OrientationVector(OrientationQuaternion(self.location, location)), OrientationVector(self.rotation))
    shieldHit = 0
    if angle >= math.pi/2: #Front Shields hit
